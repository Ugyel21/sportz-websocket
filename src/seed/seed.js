...
const REQUEST_TIMEOUT_MS = Number.parseInt(
    process.env.SEED_REQUEST_TIMEOUT_MS || "10000",
    10,
);
const RETRY_COUNT = Number.parseInt(process.env.SEED_RETRY_COUNT || "5", 10);
const RETRY_MIN_DELAY_MS = Number.parseInt(
    process.env.SEED_RETRY_MIN_DELAY_MS || "250",
    10,
);
const RETRY_MAX_DELAY_MS = Number.parseInt(
    process.env.SEED_RETRY_MAX_DELAY_MS || "1500",
    10,
);
const FORCE_LIVE =
...
function nextRetryDelay(attempt) {
    const base = RETRY_MIN_DELAY_MS * Math.pow(2, attempt);
...
async function fetchWithRetry(url, options = {}) {
    for (let attempt = 0; attempt <= RETRY_COUNT; attempt += 1) {
        ...
        try {
            ...
            if (attempt >= RETRY_COUNT || !isRetriableFetchError(error)) {
                throw error;
            }
...
async function fetchMatches(limit = 100) {
    const response = await fetchWithRetry(`${API_URL}/matches?limit=${limit}`);
    if (!response.ok) {
...
async function createMatch(seedMatch) {
    ...
    const response = await fetchWithRetry(`${API_URL}/matches`, {
        method: "POST",
